(() => {
  ...
  function buildFullPrompt(question, confirmLabel, cancelLabel){
    ...
    lines.push("");
    lines.push("Visual treatment: reads as a proper elevated panel, matching this system's other floating-panel language (Menu, Autocomplete, Popover) - background var(--graphite-900), a real 1px solid var(--line-strong) border, box-shadow \"0 8px 24px rgba(0,0,0,0.4)\", border-radius var(--radius-md). The trigger is a real clickable button styled like a small destructive/neutral button - background var(--graphite-800), border 1.5px solid var(--line-strong), color var(--text-hi), padding 6px 14px, border-radius var(--radius-sm). The arrow is a small rotated square (transform:rotate(45deg)) matching the bubble's background color. The Confirm/Cancel buttons sit in a right-aligned row below the question - Cancel is a transparent/bordered button, Confirm is a solid neutral button by default or a solid red button in the Destructive state.");
    lines.push("");
  ...
  function buildFullCode(question, confirmLabel, cancelLabel){
    ...
    lines.push(".popconfirm-demo-btn{ font-family:var(--font-body); font-size:12px; font-weight:600; padding:6px 12px; border-radius:var(--radius-sm); cursor:pointer; border:1.5px solid transparent; }");
    lines.push(".popconfirm-demo-btn--cancel{ background:transparent; border-color:var(--line-strong); color:var(--text-hi); }");
    lines.push(".popconfirm-demo-btn--confirm{ background:var(--graphite-700); color:var(--text-hi); }");
    lines.push(".popconfirm-demo-btn--confirm.is-destructive{ background:var(--red-500); color:#FFFFFF; }");
    lines.push("");
    lines.push("/* Arrow - a small rotated square, same background as the bubble */");
    lines.push(".popconfirm-demo-arrow{ flex:none; width:8px; height:8px; background:var(--graphite-900); transform:rotate(45deg); margin:-4px auto; }");
    lines.push(".popconfirm-demo-wrap--left .popconfirm-demo-arrow, .popconfirm-demo-wrap--right .popconfirm-demo-arrow{ margin:auto -4px; }");
    lines.push("");
