(() => {
  ...
  function buildFullPrompt(targetLabel, title, description){
    ...
    lines.push("");
    lines.push("Visual treatment: reads as a proper elevated panel, similar in floating-panel language to this system's Tooltip and Popover components - background var(--graphite-900), a real 1px solid var(--line-strong) border, box-shadow \"0 8px 24px rgba(0,0,0,0.4)\", border-radius var(--radius-md). The target is a decorative highlighted chip (not a real interactive control) with a visible red outline/ring - background var(--graphite-800), border 2px solid var(--red-500), box-shadow \"0 0 0 3px var(--red-tint)\" - so it reads clearly as the highlighted element the tour is pointing at. The arrow is a small rotated square (transform:rotate(45deg)) matching the card's background color. Primary actions (Next/Finish) use background var(--red-500); the Back button is a ghost/outline style.");
    lines.push("");
  ...
  function buildFullCode(targetLabel, title, description){
    ...
    lines.push(".tour-demo-btn--ghost{ background:transparent; border-color:var(--line-strong); color:var(--text-hi); }");
    lines.push(".tour-demo-btn--primary{ background:var(--red-500); color:#FFFFFF; }");
    lines.push("");
    lines.push("/* Arrow - a small rotated square, same background as the card */");
    lines.push(".tour-demo-arrow{ flex:none; width:8px; height:8px; background:var(--graphite-900); transform:rotate(45deg); margin:-4px auto; }");
    lines.push(".tour-demo-wrap--left .tour-demo-arrow, .tour-demo-wrap--right .tour-demo-arrow{ margin:auto -4px; }");
    lines.push("");
  ...
  document.addEventListener("DOMContentLoaded", function(){
    ...
    if (targetLabelInput) targetLabelInput.addEventListener("input", render);
