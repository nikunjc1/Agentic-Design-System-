(() => {
  ...
  // Patterns previously had no way to get a prompt or working code out of
  ...
  // portal. Reads each pattern's own title/description/"Built from" text
  ...
  document.addEventListener("DOMContentLoaded", function(){
    document.querySelectorAll("[data-role^='pattern-demo-']").forEach(function(demo){
      var slug = demo.getAttribute("data-role").replace("pattern-demo-", "");
      ...
      if (promptBtn){
        promptBtn.addEventListener("click", function(){
          var lines = [];
          lines.push("Create a \"" + name + "\" pattern for the Agentic Design System.");
          lines.push("");
          lines.push(description);
          lines.push("");
          lines.push("Built from: " + builtFrom + ".");
          lines.push("Use this system's own components and design tokens (color, spacing, radius, typography) for every piece - never arbitrary one-off values, since this pattern's whole point is assembling already-documented components instead of rebuilding an equivalent layout from scratch.");
      ...
      if (codeBtn){
        codeBtn.addEventListener("click", function(){
          var lines = [];
          lines.push("<!-- Agentic Design System - \"" + name + "\" pattern -->");
