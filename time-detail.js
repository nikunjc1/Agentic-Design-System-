(() => {
  ...
  var FULL_SPEC = {
    outlined: { purpose: "The default time field style - a bordered box that works well on any background and reads clearly in dense forms." },
  ...
  // Same color logic as Input's/Select's Outlined/Filled types - Time is a
  // picker whose trigger reads like a text field, so the field chrome is
  // identical, only the fixed trailing icon and the "filled" content (a
  // formatted time instead of free text) differ.
  ...
  // real .time-demo-control--h* rules in shell.css (not re-derived by
  ...
  // agree with. Same scale as Input/Select/Date Picker.
  var SIZE_SCALE = {
    "24": { pad: 10, font: 11 },
    "28": { pad: 12, font: 12 },
    "32": { pad: 12, font: 12 },
  ...
  // Size / Corner radius option lists - same value scale as Input/Select,
  ...
  // Input/Select/Date Picker: untouched-default, fully-deselected, and
  ...
  function buildFullPrompt(selectionInfo){
    ...
    var lines = [];
    lines.push("Create a complete Time component system for the Agentic Design System.");
    lines.push("");
    lines.push("Build it as ONE reusable component controlled by properties (type, size, radius, state) - not as separate one-off fields for each variant. It is visually a text field with a fixed trailing clock icon (not a leading icon, to match Select's chevron placement convention) and no visible text cursor, since its value is set via a time-selection UI rather than typed character-by-character. Scope is the trigger control itself, in all its states - the trigger opens a time-selection UI on click/focus in a real implementation, but that popup UI is not built out here, the same \"declared but not interactively simulated\" convention Date Picker itself already uses for its calendar popup.");
    lines.push("");
    lines.push("Types (2):");
    ...
    lines.push("- Filled: same appearance as Rest, showing the selected time value (e.g. 10:30 AM) instead of the placeholder.");
    lines.push("- Error: pairs with an inline error message below the field.");
    ...
    lines.push("");
    lines.push("Component properties: Label (editable text above the field), Placeholder (editable text shown when no time is chosen, e.g. \"Select a time\").");
    return lines.join("\n");
  }
  ...
  function buildFullCode(selectionInfo){
    ...
    var lines = [];
    lines.push("/* Agentic Design System - Time component (all types, states, sizes) */");
    lines.push("/* Trigger field only - the time-selection popup panel is a separate concern. */");
    lines.push(".time-field{ display:flex; flex-direction:column; gap:6px; }");
    ...
    sizesToEmit.forEach(function(size){
      ...
      lines.push(".time-control--h" + size + "{ height:" + size + "px; padding:0 " + scale.pad + "px; font-size:" + scale.font + "px; }");
    });
    lines.push("");
    ...
    radiiToEmit.forEach(function(radius){
      lines.push(".time-control--radius-" + radiusClassSuffix(radius) + "{ border-radius:" + radiusCssFor(radius) + "; }");
    });
    lines.push("");
    lines.push("/* Types (2) - rest / hover / focus / error per type */");
    TYPES.forEach(function(t){
      var css = liveCssFor(t.key);
      var cls = ".time-control--" + t.key;
    ...
    lines.push(".time-control[aria-disabled=\"true\"]{ opacity:0.5; cursor:not-allowed; pointer-events:none; }");
    lines.push("");
    ...
    var sizeClass = "time-control--h" + exampleSize;
    var radiusClassAttr = "time-control--radius-" + radiusClassSuffix(exampleRadius);
    ...
    TYPES.forEach(function(t){
      lines.push(exampleMarkupFor(t.key, sizeClass, radiusClassAttr, "Meeting time", "Select a time"));
    });
    return lines.join("\n");
  }
  ...
  document.addEventListener("DOMContentLoaded", function(){
    ...
    // Gallery-card copy CTA + click-to-navigate (time.html's listing
    // card) - separate data-role from every other system's own copy
    // roles so no script's wiring ever touches another's buttons.
    var cardCopyPromptBtn = document.querySelector('[data-role="copy-prompt-time"]');
    var cardCopyCodeBtn = document.querySelector('[data-role="copy-code-time"]');
    ...
    var timeTypeCards = document.querySelectorAll(".time-type-card[data-system]");
    timeTypeCards.forEach(function(card){
      card.setAttribute("role", "link");
      card.setAttribute("tabindex", "0");
      var href = "time-" + card.dataset.system + ".html";
    ...
    var matrixContainer = document.querySelector('[data-role="time-matrix-container"]');
    if (!matrixContainer) return;

    var defaultLabel = document.body.dataset.defaultLabel || "Meeting time";
    ...
    function buildTimeField(typeKey, stateKey, stateCls, size, radius, label, placeholder, allowClear){
      ...
      var controlClasses = "time-demo-control time-demo-control--" + typeKey + " time-demo-control--h" + size + stateCls;
      var text = isFilled ? FILLED_VALUE : (isError ? ERROR_VALUE : placeholder);
      var valueClasses = "time-demo-value" + (isFilled ? " is-filled" : "");
    ...
    if (sizeMount && window.createMultiSelect){
      propertyMultiSelects.size = window.createMultiSelect({
        ...
        labelledBy: "time-size-dropdown-label",
        onChange: render
      });
    }
    if (radiusMount && window.createMultiSelect){
      propertyMultiSelects.radius = window.createMultiSelect({
        ...
        labelledBy: "time-radius-dropdown-label",
        onChange: render
      });
    }
