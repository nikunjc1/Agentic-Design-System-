(() => {
  ...
  var FULL_SPEC = {
    outlined: { purpose: "The default dropdown trigger style - a bordered button that works well on any background and reads clearly among a group of other actions." },
    filled: { purpose: "A softer-emphasis trigger with a solid background instead of a border, useful when the button sits on a bordered card and an outline would compete visually." }
  };
  ...
  // Same color logic as Select's Outlined/Filled types (select-detail.js's
  // liveCssFor) - a Dropdown trigger is a BUTTON, not a form field, so it
  // has no Error state, and its "pressed" equivalent is Open (the menu is
  // currently visible) rather than Select's Focused-doubles-as-open pattern:
  // here Open and Focused are genuinely independent states.
  function liveCssFor(typeKey){
    ...
    if (typeKey === "outlined"){
      return {
        ...
        open: "border-color:" + red500 + ";background:" + redTint + ";",
    ...
    return {
      ...
      open: "background:" + graphite900 + ";",
      focus: "outline:2px solid " + red400 + ";outline-offset:2px;"
    };
  }

  // Hand-tuned padding/font-size per height - must stay identical to the
  // real .select-demo-control--h* / .input-demo-control--h* rules in
  // shell.css (not re-derived by formula) so Copy Code reproduces exactly
  // what the Live Preview matrix is showing, at every size, and so this
  // component's Copy Code stays visually consistent with its siblings.
  var SIZE_SCALE = {
    "24": { pad: 10, font: 11 },
    "28": { pad: 12, font: 12 },
    "32": { pad: 12, font: 12 },
  ...
  // Size / Corner radius option lists - same value scale as Select/Input,
  // shared between the page's own multi-select dropdowns and the Copy
  ...
  // Same "ask the question vs. state the explicit choice" rule as
  // Select/Input: untouched-default, fully-deselected, and "All" checked
  ...
  function buildFullPrompt(selectionInfo){
    ...
    lines.push("");
    lines.push("Build it as ONE reusable component controlled by properties (type, size, radius, state) - not as separate one-off buttons for each variant. It is a trigger button that reveals a menu of actions or commands when clicked - a real <button> element, not a form field, with no associated <label>, just its own text and a fixed trailing chevron-down icon. It is distinct from a Select trigger (which picks a form value) and from the menu panel it opens (a separate component).");
    lines.push("");
    lines.push("Types (2):");
    TYPES.forEach(function(t){
      ...
      lines.push("  Open: " + css.open + " This is the state while its menu is visible - the trigger stays visually pressed so the connection to the open menu reads clearly.");
      lines.push("  Focused: " + css.focus + " Real keyboard focus, independent of Open - never just a color change, so layout never shifts.");
    });
    lines.push("");
    lines.push("States (5, apply to every type): Rest, Hover, Open, Focused, Disabled.");
    lines.push("- Open: the menu is currently visible; the trigger takes on a pressed appearance rather than a plain hover.");
    lines.push("- Focused: a real :focus-visible outline for keyboard navigation, independent of Open.");
    lines.push("- Disabled: 50% opacity, real disabled attribute, no hover/open/focus feedback.");
    lines.push("");
  ...
  function buildFullCode(selectionInfo){
    ...
    lines.push(".dropdown-trigger{ box-sizing:border-box; display:inline-flex; align-items:center; justify-content:center; gap:8px; border-radius:8px; cursor:pointer; font-family:\"Inter\",ui-sans-serif,system-ui,sans-serif; font-weight:500; white-space:nowrap; transition:background-color .15s ease, border-color .15s ease; }");
    lines.push(".dropdown-trigger-icon{ flex:none; width:16px; height:16px; }");
    lines.push("");
    ...
    sizesToEmit.forEach(function(size){
      ...
      lines.push(".dropdown-trigger--h" + size + "{ height:" + size + "px; padding:0 " + scale.pad + "px; font-size:" + scale.font + "px; }");
    });
    lines.push("");
    ...
    radiiToEmit.forEach(function(radius){
      lines.push(".dropdown-trigger--radius-" + radiusClassSuffix(radius) + "{ border-radius:" + radiusCssFor(radius) + "; }");
    });
    lines.push("");
    lines.push("/* Types (2) - rest / hover / open / focus per type */");
    TYPES.forEach(function(t){
      var css = liveCssFor(t.key);
      var cls = ".dropdown-trigger--" + t.key;
      lines.push(cls + "{ " + css.rest + " }");
      lines.push(cls + ":hover{ " + css.hover + " }");
      lines.push(cls + ".is-open{ " + css.open + " }");
      lines.push(cls + ":focus-visible{ " + css.focus + " }");
    });
    lines.push("");
    lines.push("/* Disabled - shared by every type */");
    lines.push(".dropdown-trigger:disabled{ opacity:0.5; cursor:not-allowed; }");
    lines.push("");
    ...
    var sizeClass = "dropdown-trigger--h" + exampleSize;
  ...
  document.addEventListener("DOMContentLoaded", function(){
    ...
    // Gallery-card copy CTA + click-to-navigate (dropdown.html's listing
    // card) - separate data-role from every other system's own copy roles
    // so no script's wiring ever touches another's buttons.
    var cardCopyPromptBtn = document.querySelector('[data-role="copy-prompt-dropdown"]');
    var cardCopyCodeBtn = document.querySelector('[data-role="copy-code-dropdown"]');
    ...
    var dropdownTypeCards = document.querySelectorAll(".dropdown-type-card[data-system]");
    dropdownTypeCards.forEach(function(card){
      card.setAttribute("role", "link");
      card.setAttribute("tabindex", "0");
      var href = "dropdown-" + card.dataset.system + ".html";
    ...
    var matrixContainer = document.querySelector('[data-role="dropdown-matrix-container"]');
    if (!matrixContainer) return;

    var defaultLabel = document.body.dataset.defaultLabel || "Options";
    var labelInput = document.querySelector('[data-role="dropdown-label"]');
    var sizeMount = document.querySelector('[data-role="dropdown-size-mount"]');
    ...
    function buildDropdownTrigger(typeKey, stateKey, stateCls, size, radius, label){
      ...
      var triggerClasses = "dropdown-demo-trigger dropdown-demo-trigger--" + typeKey + " dropdown-demo-trigger--h" + size + stateCls;
    ...
    function buildMatrixSection(size, radius, label){
      var rows = STATES.map(function(state){
        var cells = TYPES.map(function(t){
          return '<td class="button-matrix-cell">' + buildDropdownTrigger(t.key, state.key, state.cls, size, radius, label) + "</td>";
        }).join("");
    ...
    if (sizeMount && window.createMultiSelect){
      propertyMultiSelects.size = window.createMultiSelect({
        ...
        labelledBy: "dropdown-size-dropdown-label",
        onChange: render
      });
    }
    if (radiusMount && window.createMultiSelect){
      propertyMultiSelects.radius = window.createMultiSelect({
        ...
        labelledBy: "dropdown-radius-dropdown-label",
        onChange: render
      });
    }
